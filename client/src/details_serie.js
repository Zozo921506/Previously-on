...
const DetailSerie = () => {
    ...
    const setWatchedEpisode = async (e, episode_id) => {
        e.preventDefault();
        ...
        try
        {
            ...
            if (response.ok)
            {
                console.log("Successfully add the episode to your watched list");
    ...
    const setWatchedEpisodePrevious = async (e, episode_id) => {
        e.preventDefault();
        const url = `https://api.betaseries.com/episodes/watched?id=${episode_id}&bulk=true`;
        try
        {
            const response = await fetch(url, {
                method: 'POST',
            ...
            if (response.ok)
            {
                console.log("Successfully add the episodes to your watched list");
                alert("L'ajout a bien été effectué");
                getSeasons();
            }
            else
                        ...
                        <ul className="episodes">
                            {seasons[seasonNumber].map((episode) => (
                                <li key={episode.id}>
                                    ...
                                    {!episode.user.seen ? (
                                        <>
                                            <button className="episodevue" onClick={(e) => setWatchedEpisode(e, episode.id)}>Episode visionné</button>
                                            <button className="addprevious" onClick={(e) => setWatchedEpisodePrevious(e, episode.id)}>Cette épisode et les précedents ont été visionnés</button>
                                        </>
                                    ) : (
                                    <>
                                        <button className='episodevue' onClick={(e) => removeWatchedEpisode(e, episode.id)}>Episode non vue</button>
                                    </>)}
                                </li>
                            ))}
                        </ul>
